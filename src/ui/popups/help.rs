...
pub fn render_help_popup(f: &mut Frame, area: Rect) {
    ...
    let help_text = vec![
        ...
        Line::from("  N (Shift+n)          Full add — deadline, priority, tags, URL, description (long)"),
        Line::from("  e                    Quick edit title"),
        Line::from("  E (Shift+e)          Full edit — all properties"),
        Line::from("  Enter / v            View detail (description, tags, deadline, URL)"),
        Line::from("  p                    Cycle priority Low→Medium→High→Urgent"),
        Line::from("  o                    Open URL in browser (from detail)"),
        ...
        Line::from("    Tab / Shift+Tab    Cycle field (Title→Desc→Deadline→Priority→Tags→URL)"),
        Line::from("    p                  Cycle priority when Priority field focused"),
        Line::from("    Enter              Save • Esc: cancel"),
        Line::from("  Description: long text fully supported — wraps in detail view (no limit)"),
        Line::from("  Deadline format: YYYY-MM-DD (e.g. 2026-12-31), empty = none"),
        Line::from("  Tags: comma-separated (e.g. work, bug, urgent)"),
        Line::from("  URL: https://... or http://... — shown as 🔗 in list, `o` to open"),
        ...
        Line::from("  JSON: { title, columns:[{name,tasks:[{id,title,description,deadline,priority,tags,url,created_at}]}], next_id }"),
        ...
        Line::from("  • Example: Set Urgent + tomorrow deadline + URL for critical tasks"),
        Line::from("  • Long descriptions: paste docs, notes — detail view wraps, no limit"),
